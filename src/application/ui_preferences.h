...
class dialog : general_options
{
  ...
  public:
        ...
        : general_options(SystemFunctions)
    {
        ...
        int shape_number  = 0;
        int current_shape = 0;
        for (scenes_t::iterator s_i = m_scenes.begin(); s_i != m_scenes.end();
             ++s_i, ++current_shape)
        {
            if (s_i->name == m_scene)
            {
        ...
        if (!m_scenes.size())
        {
            log() << error << "couldn't find any scene file in '" << rib_scene_dir()
                  << "' directory." << std::endl;
        }
        ...
        for (renderers_t::iterator r_i = m_renderers.begin(); r_i != m_renderers.end();
             ++r_i)
        {
            if (r_i->first == _3delight)
                new fltk::Item(
                    r_i->second.name.c_str(), 0, cb_renderer, (void*) _3delight);
                ...
                new fltk::Item(
                    r_i->second.name.c_str(), 0, cb_renderer, (void*) renderdotc);
        }
        s_renderer->end();
        w->add(s_renderer);
        ...
        s_renderer_code->tooltip(
            "renderer code (not editable, it's an option for the shader compiler)");
        ...
        s_pixel_filter->tooltip("select current filter used to filter the Pixel Samples "
                                "into the final color in s and t");

        s_filter_width_s = new fltk::Input(110, start + 360, 50, 23, "filter width s");
        w->add(s_filter_width_s);
        s_filter_width_s->tooltip(
            "Suggested values , Box: 1.0 Triangle: 2.0 Blackman-Harris: 2.0 Gaussian: "
            "2.5 Catmull-Rom: 4.0 Separable Catmull-Rom: 4.0 Mitchell: 4.0 Lanczos: 6.0 "
            "Bessel: 6.49 Sinc: 8.0");

        s_filter_width_t = new fltk::Input(260, start + 360, 50, 23, "filter width t");
        w->add(s_filter_width_t);
        s_filter_width_t->tooltip(
            "Suggested values , Box: 1.0 Triangle: 2.0 Blackman-Harris: 2.0 Gaussian: "
            "2.5 Catmull-Rom: 4.0 Separable Catmull-Rom: 4.0 Mitchell: 4.0 Lanczos: 6.0 "
            "Bessel: 6.49 Sinc: 8.0");
    ...
    void on_ok(fltk::Widget* W)
    {
        ...
        m_shading_rate =
            from_string(shading_rate, 1.0); // keep 1.0 (not 1) to convert to double

        std::string samples_x = trim(s_pixelsamples_x->value());
        std::string samples_y = trim(s_pixelsamples_y->value());
        m_samples_x =
            from_string(samples_x, 1.0); // keep 1.0 (not 1) to convert to double
        m_samples_y =
            from_string(samples_y, 1.0); // keep 1.0 (not 1) to convert to double
    ...
    void set_display_chooser(const std::string RendererCode, const std::string Value = "")
    {
        ...
        {
            log() << error << "unknown renderer (set_display_chooser): '" << RendererCode
                  << "'." << std::endl;
        }
        ...
        for (general_options::displays_t::const_iterator current_display =
                                                             r->second.displays.begin();
             current_display != r->second.displays.end();
             ++current_display, ++current_display_number)
        {
        ...
        const std::string Value = "")
    {
        ...
        if (m_renderers.end() == r)
        {
            log() << error << "unknown renderer (set_pixelfilter_chooser): '"
                  << RendererCode << "'." << std::endl;
        }

        s_pixel_filter->remove_all();
        s_pixel_filter->begin();
        unsigned long current_filter_number = 0;
        for (general_options::filters_t::const_iterator current_filter =
                                                            r->second.filter_type.begin();
             current_filter != r->second.filter_type.end();
             ++current_filter, ++current_filter_number)
        {
