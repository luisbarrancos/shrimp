...
class dialog
{
  ...
  public:
        ...
        : m_shader_block(Block)
    {
        ...
        for (types_t::const_iterator st_i = m_storage_types.begin();
    ...
    void edit_output(const std::string& OutputName)
    {
        ...
        if (type == "array")
        {
            const std::string array_type =
                m_shader_block->get_output_type_extension(m_edited_output);
            int type_number = 0;
            for (types_t::const_iterator t_i = m_array_types.begin();
                 t_i != m_array_types.end();
                 ++t_i, ++type_number)
            ...
            const int array_size =
                m_shader_block->get_output_type_extension_size(m_edited_output);
    ...
    void on_ok(fltk::Widget* W)
    {
        ...
        {
            m_shader_block
                ->set_output_storage(m_edited_output, storage_list[storage_type_number]);
        }
