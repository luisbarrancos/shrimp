...
static void cb_colour_chooser(fltk::Widget* w, void* v)
{
    ...
    new_colour << ((float) r / 255.0) << " " << ((float) g / 255.0) << " "
               << ((float) b / 255.0);
...
static void cb_file_chooser(fltk::Widget* w, void* v)
{
    ...
    // choose shader file
    const char* file =
        fltk::file_chooser("Open Texture", "*.{tif,tiff}\t*.{tdl,tex,tx}", result);
    if (!file)
    {
        // cancel
        return;
...
class dialog
{
  ...
  public:
        ...
        : m_shader_block(Block)
    {
        ...
        for (storages_t::const_iterator st_i = m_storage_types.begin();
             st_i != m_storage_types.end();
             ++st_i)
        {
        ...
        for (types_t::const_iterator t_i = m_array_types.begin();
             t_i != m_array_types.end();
             ++t_i)
        {
            new fltk::Item(t_i->c_str());
        }
    ...
    void edit_input(const std::string& InputName)
    {
        ...
        for (storages_t::const_iterator s_i = m_storage_types.begin();
             s_i != m_storage_types.end();
             ++s_i, ++storage_type_number)
        {
            if (storage == *s_i)
        ...
        int type_number        = 0;
        for (types_t::const_iterator t_i = m_types.begin(); t_i != m_types.end();
             ++t_i, ++type_number)
        {
            if (type == *t_i)
            {
        ...
        if (type == "array")
        {
            const std::string array_type =
                m_shader_block->get_input_type_extension(m_edited_input);
            int type_number = 0;
            for (types_t::const_iterator t_i = m_array_types.begin();
                 t_i != m_array_types.end();
                 ++t_i, ++type_number)
            ...
            const int array_size =
                m_shader_block->get_input_type_extension_size(m_edited_input);
    ...
    void on_ok(fltk::Widget* W)
    {
        ...
        {
            m_shader_block
                ->set_input_storage(m_edited_input, storage_list[storage_type_number]);
        }
    ...
    static void cb_type_change(fltk::Widget* W, void* Data)
    {
        ((dialog*) Data)->on_type_change(W);
    }
