    ...
    , m_block_menu(20, 22, 90, 24, "Add block")
{
    ...
    menu_file_shader_properties
        ->callback((fltk::Callback*) cb_menu_shader_properties, this);
...
*/
    ...
    for (block_tree_node_list_t::const_iterator tree_node = root.child_nodes.begin();
         tree_node != root.child_nodes.end();
         ++tree_node)
    {
    ...
    preview_button->tooltip(
        "Click to preview the shader using selected renderer and display");
...
void application_window::on_scene_choice(fltk::Widget* W, void* Data)
{
    ...
    for (general_options::scenes_t::const_iterator s = m_scenes.begin();
         s != m_scenes.end();
         ++s)
    {
...
void application_window::build_menu(const block_tree_node_t& tree_node)
{
    ...
    for (block_tree_node_list_t::const_iterator sub_node = tree_node.child_nodes.begin();
         sub_node != tree_node.child_nodes.end();
         ++sub_node)
    {
        build_menu(*sub_node);
    }

    for (default_block_list_t::const_iterator block = tree_node.blocks.begin();
         block != tree_node.blocks.end();
         ++block)
    {
...
void application_window::set_renderer_chooser_value(const std::string RendererCode)
{
    ...
    for (general_options::renderers_t::iterator r_i = m_renderers.begin();
         r_i != m_renderers.end();
         ++r_i, ++current_menu_number)
    {
...
std::string application_window::set_display_chooser_value(
...
void application_window::set_scene_chooser_value(const std::string Scene)
{
    ...
    for (general_options::scenes_t::iterator s_i = m_scenes.begin();
         s_i != m_scenes.end();
         ++s_i, ++current_menu_number)
    {
...
void application_window::save_scene_as()
{
    ...
    // choose shader file
    const char* file =
        fltk::file_chooser("Save Shader As", "Shrimp file (*.xml)", result);
    if (!file)
        return;
    ...
    if (std::ifstream(real_file_name.c_str()))
    {
        const std::string message =
            "Do you want to overwrite '" + std::string(fltk::filename_name(real_file_name.c_str())) + "'?";
