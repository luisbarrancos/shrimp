...
{
    auto i = m_block_selection.find(Block);
...
{
    auto i = m_group_selection.find(Group);
...
{
    log() << aspect << "services: set_block_selection of " << Block->name() << " with "
          << Selection << std::endl;
...
{
    log() << aspect << "services: set_block_rolled_state of " << Block->name() << " with "
          << Rolled << std::endl;
...
void services::copy_blocks(const std::string& Name, const int Group)
{
    ...
    std::string NewName;
    // create block
    shader_block* new_block = new shader_block(Temp, "", false);
    ...
    {
        for (auto& m_input : BlockToCopy->m_inputs)
        {
            new_block->add_input(
                m_input.m_name,
                m_input.get_type(),
                m_input.get_type_extension(),
                m_input.get_storage(),
                m_input.m_description,
                m_input.get_value(),
                m_input.m_multi_operator,
                m_input.m_shader_parameter);
        }
    }

    if (BlockToCopy->m_outputs.size())
    {
        for (auto& m_output : BlockToCopy->m_outputs)
        {
            new_block->add_output(
                m_output.m_name,
                m_output.get_type(),
                m_output.get_type_extension(),
                m_output.get_storage(),
                m_output.m_description,
                m_output.m_shader_output);
        }
    }
...
{
    // copy connections
    for (auto& connection : m_scene->m_dag)
    {
        const copy_block_t to =
            (std::make_pair(connection.first.first, get_block(connection.first.first)));
        const copy_block_t from =
            (std::make_pair(connection.second.first, get_block(connection.second.first)));

        // found connections to copy
        auto i                        = m_copy_selection.find(to);
        ...
        {
            const shrimp::io_t to_copy =
                (std::make_pair(i->second.first, connection.first.second));

            auto j = m_copy_selection.find(from);
            ...
            {
                const shrimp::io_t from_copy =
                    (std::make_pair(j->second.first, connection.second.second));
...
{
    // pasted blocks
    for (auto& new_block : m_copy_buffer)
    {
        ...
        auto g = m_groups_buffer.find(paste_name);
    ...
    for (auto& connection : m_scene->m_dag_copy)
    {
...
void services::copy_selected_blocks(shader_block* block)
{
    ...
    {
        // multi-selection
        for (auto block_copy : m_block_selection)
        {
...
void services::paste_buffered_blocks()
{
    ...
    if (m_copy_selection.size())
    {
        ...
        for (auto& new_block : m_copy_buffer)
        {
            ...
            {
                set_block_selection(new_block.second.second, true);
            }
        }

        // make group copy current selection
        for (auto& g : m_groups_buffer)
        {
            set_group_selection(g.second, true);
        }
    }
...
void services::delete_selection()
{
    ...
    for (auto block_i : m_block_selection)
    {
    ...
    for (int group_i : m_group_selection)
    {
...
void services::build_xml_documentation()
{
    ...
    {
        build_xml_documentation_block(child_node, index_xml, content_xml);
    }
    ...
    if (m_system_functions->is_directory("doc"))
    {
        ...
        std::string content_path =
            m_system_functions->combine_paths("doc", "content.xml");
...
void services::build_xml_documentation_block(
    ...
    std::string& content)
{
    ...
    {
        build_xml_documentation_block(child_node, index, content);
    }

    for (const auto& block : tree_node.blocks)
    {
