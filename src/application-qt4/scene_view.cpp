...
{
    const double size =
        opengl_view::fit_scene(currentSize.rwidth(), currentSize.rheight());
    redraw();
    ...
    else
    {
        ...
        if (event->buttons() & Qt::LeftButton)
        {
            mouse_left_button_drag(
                currentSize.width(), currentSize.height(), event->modifiers() & Qt::ALT);
        }
    }
...
{
    log() << aspect << "mouseReleaseEvent, released button = " << event->button()
          << std::endl;
