    ...
    , preferences(prefs)
{
    ...
    log() << aspect << "Successfully loaded " << successful_blocks << " blocks."
          << std::endl;
...
void scene::new_scene()
{
    ...
    m_rib_root_block =
        new rib_root_block(unique_name, this, m_system_functions, preferences);
...
void scene::load_default_blocks(block_tree_node_t& RootNode, unsigned long& BlockCount)
{
    ...
    std::vector<std::string> directory_content =
        m_system_functions->list_directory(RootNode.node_path);

    // skip empty directories
    if (directory_content.size() == 0)
    {
        log() << error << "tried to load blocks from empty directory '"
              << RootNode.node_path << "'." << std::endl;
        return;
    }
    ...
    for (auto file : directory_content)
    {
        const std::string file_path =
            m_system_functions->combine_paths(RootNode.node_path, file);
    ...
    auto path_i = file_paths.begin();
    auto file_i = files.begin();
    ...
    // process blocks
    for (auto& block_path : block_paths)
    {
        // try loading the block
        shader_block* new_block = builder.build_block(block_path);
        if (new_block)
        {
            ...
            {
                // duplicate name
                log() << error << "couldn't load " << block_path << " : a block named '"
                      << new_block->name() << "' already exists." << std::endl;
            }
            else
            {
                default_block_t block_info;
                block_info.name = new_block->name();
                block_info.path = block_path;
        ...
        {
            log() << error << "couldn't load " << block_path << std::endl;
        }
    }

    log() << aspect << " loaded " << RootNode.blocks.size() << " blocks from "
          << RootNode.node_path << std::endl;
}
...
{
    if (auto* rib_block = dynamic_cast<rib_root_block*>(m_rib_root_block))
    {
    ...
    std::string& ParentOutput) const
{
    ...
    for (const auto& connection : m_dag)
    {
        ...
        if (to == input)
        {
            ...
            auto block              = m_blocks.find(from.first);
