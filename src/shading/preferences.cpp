...
void general_options::initialize(
...
bool general_options::load()
{
    ...
    {
        log() << aspect << "Preferences file not found (" << file
              << "), creating the default one." << std::endl;
        save();
    ...
    else
    {
        for (TiXmlNode* c = xml_prefs.FirstChild()->FirstChild(); c; c = c->NextSibling())
        {
            ...
            if (element == "renderer")
            {
                for (TiXmlAttribute* a = c->ToElement()->FirstAttribute(); a;
                     a                 = a->Next())
                {
            ...
            else if (element == "output")
            {
                     ...
                     a                 = a->Next())
                {
                    ...
                    {
                        m_shading_rate = from_string(
                            trim(a->Value()),
                            256.0); // keep 256.0 (not 256) to convert to double
                    }
                    else if (name == "samples_x")
                    {
                        m_samples_x = from_string(
                            trim(a->Value()),
                            1.0); // keep 1.0 (not 1) to convert to double
                    }
                    else if (name == "samples_y")
                    {
                        m_samples_y = from_string(
                            trim(a->Value()),
                            1.0); // keep 1.0 (not 1) to convert to double
                    }
                    ...
                    {
                        m_filter_width_s = from_string(
                            trim(a->Value()),
                            8.0); // keep 1.0 (not 1) to convert to double
                    }
                    else if (name == "filter_width_t")
                    {
                        m_filter_width_t = from_string(
                            trim(a->Value()),
                            8.0); // keep 1.0 (not 1) to convert to double
                    }
                    else if (name == "scene")
        ...
        log() << aspect << "   shader extension : " << m_compiled_shader_extension
              << std::endl;
...
void general_options::set_defaults()
{
    ...
    {
        auto renderer = m_renderers.find("3Delight");
    ...
    {
        m_shader_compiler           = "shaderdl -DRENDERER=%r -I%i -o %o %s";
        m_compiled_shader_extension = "sdl";
        m_renderer_code             = "_3delight";
        m_renderer                  = "renderdl %s";
...
std::string general_options::get_RIB_scene()
{
    ...
    std::vector<std::string> scene_files =
        m_system_functions->list_directory(m_rib_scene_dir);

    std::string scene_path("");
    for (auto file : scene_files)
    {
        const std::string file_path =
            m_system_functions->combine_paths(m_rib_scene_dir, file);
    ...
    if (scene_path.size())
    {
        ...
        if (!scene_file)
        {
            log() << error << "couldn't open RIB scene template '" << scene_path
                  << "' file." << std::endl;
        }
        else
        {
...
void general_options::load_renderer_list()
{
    ...
    {
        log() << error << "couldn't load renderer file '" << renderer_file()
              << "' (not found or malformed)." << std::endl;
        return;
    }
    ...
    for (TiXmlNode* n = xml_renderers.FirstChild()->FirstChild(); n; n = n->NextSibling())
    {
        ...
        for (TiXmlNode* c = n->FirstChild(); c; c = c->NextSibling())
        {
            ...
            {
                new_renderer.compiled_shader_extension =
                    c->FirstChild()->ToText()->Value();
            }
            ...
            {
                // Get attributes
                for (TiXmlAttribute* a = n->ToElement()->FirstAttribute(); a;
                     a                 = a->Next())
                {
                    const std::string name(a->Name());
                    if (name == "command")
                ...
                for (TiXmlNode* c = n->ToElement()
                                        ->FirstChild("texture")
                                        ->FirstChild("texture_switches")
                                        ->FirstChild("filter")
                                        ->FirstChild("filter_type")
                                        ->FirstChild();
                     c;
                     c = c->NextSibling())
                {
...
void general_options::load_scene_list()
{
    ...
    log() << aspect << "Loading " << scene_files.size() << " scene files from "
          << m_rib_scene_dir << std::endl;
    for (auto file : scene_files)
    {
        const std::string file_path =
            m_system_functions->combine_paths(rib_scene_dir(), file);
        if (!m_system_functions->is_directory(file_path))
        {
            const std::string extension = m_system_functions->get_file_extension(file);
            if (extension == "rib")
...
void general_options::set_renderer(const std::string& RendererCode)
{
    ...
    auto r = m_renderers.find(RendererCode);
...
{
    auto renderer = m_renderers.find(m_renderer_code);
    if (renderer == m_renderers.end())
    {
    ...
    for (const auto& display : renderer->second.displays)
    {
        m_renderer_display = display;
    }
}
