...
shader_block* scene::add_predefined_block(const std::string& BlockName)
{
    ...
    auto block_i = m_default_blocks.find(BlockName);
...
void scene::delete_block(const std::string& BlockName)
{
    ...
    if (block == nullptr || block->m_root_block)
    {
...
void scene::delete_group(const int Group)
{
    ...
    {
        const shader_block* blockSel = m_block.second;
...
shader_block* scene::get_root_block()
{
    for (auto& m_block : m_blocks)
    {
        if (m_block.second->m_root_block)
...
void scene::set_block_name(shader_block* Block, const std::string& NewName)
{
    ...
    // update group (if any)
    auto block_group = m_groups.find(old_name);
...
void scene::connect(const shrimp::io_t& Input, const shrimp::io_t& Output)
{
    ...
    if ((input_block && output_block) || (m_dag == m_dag_copy))
    {
        if ((input_block->is_input(Input.second) && output_block->is_output(Output.second)))
        {
            if (input_block->is_input_multi_operator(Input.second))
            {
                ...
                else
                {
                    ...
                    const std::string new_input =
                        input_block->add_multi_input(Input.second);
                    // connect to that new input
                    m_dag.insert(
                        std::make_pair(shrimp::io_t(Input.first, new_input), Output));
                }
            ...
            else
            {
                ...
                {
                    if (!output_block->set_output_type(
                            Output.second, input_block->get_input_type(Input.second)))
                    {
                        log()
                            << warning
                            << "connecting to an input that doesn't have a valid type: '"
                            << input_block->get_input_type(Input.second) << "' from "
                            << input_block->name() << " -> " << Input.second << std::endl;
                    }
                }
                else
                {
                    ...
                    if (dynamic_cast<rib_root_block*>(input_block))
                    {
                        if (Input.second == "AOV")
                        {
                            input_block->set_input_type(
                                Input.second,
                                output_block->get_output_type(Output.second));
                        }
                    }
                }
    ...
    log() << error << "connection failed : " << Input.first << "::" << Input.second
          << " must be an input, and " << Output.first << "::" << Output.second
          << " must be an output" << std::endl;
}

void scene::disconnect(const shrimp::io_t& IO)
{
    // check whether IO is an input
    auto connection = m_dag.find(IO);
...
void scene::upward_blocks(shader_block* StartingBlock, shrimp::shader_blocks_t& List)
{
    ...
    for (auto input = StartingBlock->m_inputs.begin();
         input != StartingBlock->m_inputs.end();
         ++input)
    {
        // get input's parent
        std::string output_name;
        shader_block* new_block =
            get_parent(StartingBlock->name(), input->m_name, output_name);
