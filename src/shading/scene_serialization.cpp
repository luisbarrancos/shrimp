...
bool scene::load(const std::string& Scene)
{
    ...
    {
        log() << error << "couldn't load shader " << Scene << ", is it a valid XML file?"
              << std::endl;
        return false;
    }
    ...
    TiXmlNode* network_xml_node = nullptr;

    for (TiXmlAttribute* a = xml_shader.FirstChild()->ToElement()->FirstAttribute(); a;
         a                 = a->Next())
    {
        // parse attributes
        ...
        {
            log() << error << "unhandled shader attribute : '" << name << "'"
                  << std::endl;
        }
    }
    ...
    for (TiXmlNode* c = xml_shader.FirstChild()->FirstChild(); c; c = c->NextSibling())
    {
        ...
        {
            log() << error << "unknown shader element '" << element << "' in " << Scene
                  << std::endl;
        }
    }
    ...
    for (TiXmlNode* e = network_xml_node->FirstChild(); e; e = e->NextSibling())
    {
        ...
        if (element == "block")
        {
            ...
            // create block
            shader_block* new_block = nullptr;
            if (!root_block)
            {
            ...
            else
            {
                //if (root_value == "RIB") {
                m_rib_root_block =
                    new rib_root_block(id, this, m_system_functions, preferences);
                ...
                for (TiXmlNode* c = e->FirstChild(); c; c = c->NextSibling())
                {
                    ...
                    if (element == "rib_statements")
                    {
                        if (c->FirstChild())
                        {
                            ...
                            m_rib_root_block->set_general_statements(
                                trim(statements) + "\n");
                        }
                    }
...
*/
        ...
        {
            log() << error << "Unknown element '" << element << "' in " << Scene
                  << "'s network." << std::endl;
            continue;
        }
    }
    ...
    for (TiXmlNode* e = network_xml_node->FirstChild(); e; e = e->NextSibling())
    {
        ...
        for (TiXmlNode* c = e->FirstChild(); c; c = c->NextSibling())
        {
            ...
            if (c->FirstChild())
            {
                ...
                if (!xml::get_attribute(
                        *c->FirstChild()->ToElement(), "output", output_name))
                    continue;

                std::string output_id;
                if (!xml::get_attribute(
                        *c->FirstChild()->ToElement(), "parent", output_id))
                    continue;
...
xml::element scene::xml_network()
{
    ...
    for (auto& m_block : m_blocks)
    {
        ...
        shader_block* block = m_block.second;
        ...
        {
            if (auto* rib_block = dynamic_cast<rib_root_block*>(block))
            {
        ...
        for (auto input = block->m_inputs.begin(); input != block->m_inputs.end();
             ++input)
        {
            ...
            {
                xml_input.push_attribute(
                    "type_extension",
                    input->get_type_extension() + ":" + string_cast(input->get_type_extension_size()));
            }
            ...
            {
                xml_input
                    .push_attribute("multi_parent", input->m_multi_operator_parent_name);
            }
            ...
            auto connection = m_dag.find(shrimp::io_t(block->name(), input->m_name));
        ...
        {
            const std::string name = m_output.m_name;
            xml::element xml_output("output");
            xml_output.push_attribute("name", name);
            xml_output.push_attribute("type", m_output.get_type());
            if (m_output.get_type() == "array")
            {
                xml_output.push_attribute(
                    "type_extension",
                    m_output.get_type_extension() + ":" + string_cast(m_output.get_type_extension_size()));
            }
            xml_output.push_attribute("storage", m_output.get_storage());
            xml_output.push_attribute("description", m_output.m_description);
            if (m_output.m_shader_output)
            {
    ...
    for (int group : groups)
    {
        ...
        xml_group.push_attribute("id", group);
        xml_group.push_attribute("name", get_group_name(group));
        for (auto& m_group : m_groups)
        {
            if (m_group.second == group)
            {
                xml::element block("block");
                block.push_attribute("name", m_group.first);
