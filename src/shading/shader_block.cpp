...
int property::add_possible_types(const std::string& Types)
{
    ...
    if (Types.size())
    {
        const std::string& type_list = Types;
...
bool property::set_type(const std::string& Type)
{
    ...
    if (Type.size())
    {
        const std::string& token_list = Type;
...
bool property::set_type_extension(const std::string& TypeExtension)
{
    ...
    if (get_type() == "array")
    {
        ...
        if (new_type == UNKNOWN)
        {
            log() << error << "unknown type extension '" << m_current_type_extension
                  << "'" << std::endl;
            return false;
        }
...
property::variable_t property::convert_type(const std::string& Type)
{
    ...
    log() << warning << "unknown type (convert_type) : '" << Type << "' for property '"
          << m_name << "'" << std::endl;
...
std::string property::convert_type(variable_t Type) const
{
    ...
    default:
        log() << warning << "unknown type (convert_type) for property '" << m_name
              << "'" << std::endl;
        return "unknown";
    }
}
...
property::storage_t property::convert_storage(const std::string& Storage)
{
    ...
    if (Storage.size())
    {
        log() << warning << "empty storage (convert_storage) for property '" << m_name
              << "'" << std::endl;
    }
...
std::string property::convert_storage(storage_t Storage) const
{
    ...
    default:
        log() << warning << "unhandled storage (convert_storage) for property '"
              << m_name << "'" << std::endl;
        return "varying";
    }
}
...
{
    types_t list;
    list.emplace_back("varying");
    list.emplace_back("uniform");
    ...
    const bool ShaderParameter)
{
    ...
    if (!ok)
    {
        log() << warning << "...above warnings about the '" << Name << "' input of the '"
              << name() << "' block!" << std::endl;
    }
...
std::string shader_block::add_multi_input(const std::string& ParentName)
{
    ...
    {
        log() << error << "input '" << ParentName << "' doesn't exist in the '" << name()
              << "' block!" << std::endl;
        return "";
    }
...
void shader_block::add_output(
    const std::string& Name,
    const std::string& Type,
    const std::string& TypeExtension,
    const std::string& Storage,
    const std::string& Description,
    const bool ShaderOutput)
{
    bool ok = true;
    ...
    if (!ok)
    {
        log() << warning << "...above warnings about the '" << Name << "' output of the '"
              << name() << "' block!" << std::endl;
    }
...
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.get_type();
        }
    }
...
std::string shader_block::output_type(const std::string& Name) const
{
    ...
    {
        if (m_output.m_name == Name)
        {
            return m_output.get_type();
        }
    }
...
void shader_block::set_input_value(const std::string& Name, const std::string& Value)
{
    ...
    log() << error << "unmatched shader block input '" << Name << "' in " << name()
          << std::endl;
}
...
std::string shader_block::get_input_value_as_sl_string(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.value_as_sl_string();
        }
    }
...
bool shader_block::set_input_type_extension(
    const std::string& Name,
    const std::string& TypeExtension)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.set_type_extension(TypeExtension);
        }
    }
...
std::string shader_block::get_input_type_extension(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.get_type_extension();
        }
    }
...
int shader_block::get_input_type_extension_size(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.get_type_extension_size();
        }
    }
...
bool shader_block::set_input_storage(const std::string& Name, const std::string& Storage)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.set_storage(Storage);
        }
    }
...
void shader_block::set_input_parent(const std::string& Name, const std::string& Parent)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            m_input.m_multi_operator_parent_name = Parent;
            return;
        }
    }
...
std::string shader_block::get_input_parent(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.m_multi_operator_parent_name;
        }
    }

    log() << error << "shader block input '" << Name << "' in " << name()
          << " has no parent." << std::endl;
    return "";
}

void shader_block::set_input_type_parent(
    const std::string& Name,
    const std::string& Parent)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            m_input.set_type_parent(Parent);
            return;
        }
    }
...
bool shader_block::set_input_description(
    const std::string& name,
    const std::string& description)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == name)
        {
            m_input.setDescription(description);
            return true;
        }
    }

    log() << error << "unmatched shader block input '" << name << "' in " << this->name()
          << std::endl;
    return false;
}
...
bool shader_block::set_output_type(const std::string& Name, const std::string& Type)
{
    ...
    for (auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            ...
            for (auto& m_input : m_inputs)
            {
                const std::string parent = m_input.get_type_parent();
                if (!parent.empty() && (parent == Name))
                {
                    answer &= m_input.set_type(Type);

                    // if it's a multi-input, change the children's type
                    if (m_input.is_multi_operator())
                    {
                        // get list of child inputs
                        std::vector<std::string> children;
                        get_multi_input_child_list(m_input.m_name, children);
            ...
            return answer && m_output.set_type(Type);
        }
    }

    log() << error << "unmatched shader block output '" << Name << "' in " << name()
          << std::endl;
    return false;
}
...
bool shader_block::set_output_type_extension(
    const std::string& Name,
    const std::string& TypeExtension)
{
    for (auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            return m_output.set_type_extension(TypeExtension);
        }
    }
...
std::string shader_block::get_output_type_extension(const std::string& Name) const
{
    for (const auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            return m_output.get_type_extension();
        }
    }
...
int shader_block::get_output_type_extension_size(const std::string& Name) const
{
    for (const auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            return m_output.get_type_extension_size();
        }
    }
...
bool shader_block::set_output_storage(const std::string& Name, const std::string& Storage)
{
    ...
    for (auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            return m_output.set_storage(Storage);
        }
    }
...
bool shader_block::set_output_description(
    const std::string& name,
    const std::string& description)
{
    for (auto& m_output : m_outputs)
    {
        if (m_output.m_name == name)
        {
            m_output.setDescription(description);
            return true;
        }
    }

    log() << error << "unmatched shader block output '" << name << "' in " << this->name()
          << std::endl;
    return false;
}
...
bool shader_block::is_shader_output(const std::string& Name) const
{
    for (const auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            return m_output.m_shader_output;
        }
    }
...
void shader_block::set_shader_output(const std::string& Name, const bool State)
{
    for (auto& m_output : m_outputs)
    {
        if (m_output.m_name == Name)
        {
            m_output.m_shader_output = State;
            return;
        }
    }
...
bool shader_block::is_shader_parameter(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.m_shader_parameter;
        }
    }
...
void shader_block::set_shader_parameter(const std::string& Name, const bool State)
{
    for (auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            m_input.m_shader_parameter = State;
            return;
        }
    }
...
bool shader_block::is_input_multi_operator(const std::string& Name) const
{
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_name == Name)
        {
            return m_input.is_multi_operator();
        }
    }
...
bool shader_block::set_input_multi_operator_parent(
    const std::string& Name,
    const std::string& Parent)
{
    for (auto& m_input : m_inputs)
...
void shader_block::get_multi_input_child_list(
    const std::string& Name,
    std::vector<std::string>& Children) const
{
    ...
    for (const auto& m_input : m_inputs)
    {
        ...
        else if (m_input.m_multi_operator_parent_name == Name)
        {
            Children.push_back(m_input.m_name);
        }
    }

    if (!found)
    {
        log() << error << "input '" << Name
              << "' not found while listing its child inputs." << std::endl;
    }
}
...
unsigned long shader_block::input_count() const
{
    ...
    for (const auto& m_input : m_inputs)
    {
        if (m_input.m_multi_operator_parent_name.empty())
        {
            ++count;
        }
    }
...
void shader_block::dump_structure() const
{
    ...
    for (const auto& m_input : m_inputs)
    {
        const std::string input_name = m_input.m_name;
        std::cerr << "	" << input_name << " (" << input_type(input_name)
                  << ") = " << get_input_value(input_name) << std::endl;
        if (!m_input.m_multi_operator.empty())
            std::cerr << "    Multi : " << m_input.m_multi_operator << std::endl;
    }

    std::cerr << "  output properties:" << std::endl;
    for (const auto& m_output : m_outputs)
    {
        const std::string output_name = m_output.m_name;
        std::cerr << "	" << output_name << " (" << output_type(output_name) << ") = "
                  << "type" << std::endl;
    }
}
...
bool shader_block::load_from_xml(TiXmlNode& XML)
{
    ...
    for (TiXmlAttribute* a = XML.ToElement()->FirstAttribute(); a; a = a->Next())
    {
        ...
        else if (name == "rolled")
        {
            ...
            name == "id" || name == "position_x" || name == "position_y" || name == "root")
        {
            // ignore them
        }
        else
        {
            log() << warning << "unhandled block attribute : '" << name << "'."
                  << std::endl;
        }
    }
        ...
        if (element == "input")
        {
            ...
            for (TiXmlAttribute* a = c->ToElement()->FirstAttribute(); a; a = a->Next())
            {
                ...
                else if (name == "type_parent")
                {
                    ...
                    log() << error << "unhandled input attribute : '" << name << "'"
                          << std::endl;
            }
            ...
            add_input(
                input_name,
                input_type,
                input_type_extension,
                input_storage,
                input_description,
                input_value,
                input_multi_operator,
                shader_parameter_input);
        ...
        else if (element == "output")
        {
            ...
            for (TiXmlAttribute* a = c->ToElement()->FirstAttribute(); a; a = a->Next())
            {
                ...
                else if (name == "shader_output")
                {
                    ...
                    log() << error << "unhandled output attribute : '" << name << "'"
                          << std::endl;
            }
...
shader_block* shader_block_builder::build_block(const std::string& FilePath)
{
    ...
    {
        log() << error << "couldn't load block " << FilePath
              << ", is it a valid XML file?" << std::endl;
        return nullptr;
    }
...
*/
    std::string type("");
    for (TiXmlAttribute* a = xml_block.FirstChild()->ToElement()->FirstAttribute(); a;
         a                 = a->Next())
    {
        const std::string name(a->Name());
        if (name == "type")
    ...
    if (type != "function")
    {
        log() << error << "unrecognized block type : '" << type << "' in " << FilePath
              << std::endl;
        return nullptr;
    }
