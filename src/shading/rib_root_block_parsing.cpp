...
using string_pos = std::string::size_type;
...
void new_code_line(string_pos n, std::string& new_code)
{
    ...
    {
        for (const auto& new_instanciation : new_instanciations)
        {
            new_code_lines.push_back(new_instanciation);
        }
    }
...
std::string get_valued_variable_name(const std::string& name)
{
    return "valued_variable_" + name;
}

void add_declaration(const std::string& name, std::set<std::string>& local_declarations)
{
    ...
    std::set<std::string>& local_declarations)
{
    ...
    string_pos shrimp_variable_real_start = 0;
    string_pos shrimp_variable_start      = 0;
    string_pos shrimp_variable_end        = 0;
    ...
    for (std::string::size_type n = 0; n < code.size(); ++n)
    {
        char c = code[n];
        new_code += c;
        if (c == ' ' || c == '\n' || c == '\t' || c == '\r' || single_line_comment || multi_line_comment)
        {
        ...
        else if (c == ')')
        {
            ...
            if (in_shrimp_variable)
            {
                ...
                shrimp_variable     = trim(std::string(
                    code,
                    shrimp_variable_start,
                    shrimp_variable_end - shrimp_variable_start + 1));
            }
        }
        ...
        else if (c == ']')
        {
            if (in_shrimp_variable_array_index)
            {
                ...
                string_pos replacement_start =
                    new_code.size() - (n - shrimp_variable_real_start + 1);
                string_pos replacement_size =
                    shrimp_variable_end - shrimp_variable_real_start + 2;

                new_code.replace(
                    replacement_start,
                    replacement_size,
                    get_valued_variable_name(shrimp_variable));
            }
        }
    ...
    for (auto& new_code_line : new_code_lines)
    {
        shader_block += "\t" + new_code_line;
    }
    ...
    for (std::string::size_type n = 0; n < array.size(); ++n)
    {
        ...
        if (c == ',')
        {
            if (parenthesis_level == 0)
            {
                ...
                std::string element =
                    std::string(array, element_start, element_end - element_start + 1);
    ...
    std::set<std::string>& local_declarations)
{
    ...
    for (std::string::size_type n = 0; n < code.size(); ++n)
    {
        ...
        else if (c == '}')
        {
            if (in_array_initialization)
            {
                ...
                // store l-value
                std::string lvalue = std::string(
                    code,
                    current_code_line_start,
                    lvalue_end - current_code_line_start + 1);
                lvalue = trim(lvalue);

                // put array in a string
                string_pos array_end = n - 1;
                std::string array =
                    std::string(code, array_start, array_end - array_start + 1);

                string_pos replacement_start = current_code_line_start;
                string_pos replacement_size  = array_end - replacement_start + 2;

                std::string replacement =
                    std::string(code, replacement_start, replacement_size);
    ...
    // build shader block
    std::string shader_block = "";
    for (auto line : new_code_lines)
    {
        // quick'n'dirty replacements
        for (auto& replacement : replacements)
        {
            std::string orig   = replacement.first;
            std::string array  = replacement.second;
            ...
            if (repl_start != std::string::npos)
            {
                ...
                for (auto& array_value : array_values)
                {
                    ...
                    new_array += "\t" + lvalue + "[" + string_cast(array_index++) + "] = " + array_value;
                }
