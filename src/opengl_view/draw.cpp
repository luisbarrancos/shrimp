...
void opengl_view::draw_shader(int window_width, int window_height)
{
    ...
    for (auto block : block_list)
    {
        ...
        if (!group)
        {
            ...
            {
                draw_block(
                    block, block->m_position_x, block->m_position_y, property_positions);
            }
        }
    }
    ...
    for (auto& connection : scene_dag)
    {
        ...
        const auto to_property = property_positions.find(to);
        if (property_positions.end() == to_property)
        {
            ...
            if (!parent.empty())
            {
                ...
                const auto parent_property =
                    property_positions.find(shrimp::io_t(to.first, parent));
                if (property_positions.end() == parent_property)
                {
                    log() << error << "parent property '" << parent
                          << "' not found in block '" << to.first << "'" << std::endl;
                }
                else
                {
            ...
            else
            {
                ...
                if (block_group)
                {
                    ...
                    auto p = m_group_positions.find(block_group);
                    if (p == m_group_positions.end())
                    {
                        log() << error << "group '" << block_group << "' not found."
                              << std::endl;
                        continue;
                    }
                ...
                {
                    log() << error << "start property '" << to.first << "-" << to.second
                          << "' not found." << std::endl;
                    continue;
                }
            }
        ...
        const auto from_property = property_positions.find(from);
        if (property_positions.end() == from_property)
        {
            ...
            {
                log() << error << "end property '" << from.first << "-" << from.second
                      << "' not found." << std::endl;
                continue;
            }
        }
        ...
        GLfloat ctrlpoints[6][3] = {{static_cast<GLfloat>(from_x + 0.1), from_y, 0.0},
                                    {static_cast<GLfloat>(from_x + 0.25), from_y, 0.0},
                                    {static_cast<GLfloat>(from_x + 1.0), from_y, 0.0},
                                    {static_cast<GLfloat>(to_x - 1.1), to_y, 0.0},
                                    {static_cast<GLfloat>(to_x - 0.35), to_y, 0.0},
                                    {static_cast<GLfloat>(to_x - 0.1), to_y, 0.0}
    ...
    {
        const auto start_property = property_positions.find(m_connection_start);
        if (property_positions.end() == start_property)
        {
            log() << error << "connection start property '" << m_connection_start.first
                  << "-" << m_connection_start.second << "' not found." << std::endl;
        }
        else
        {
            ...
            double to_y =
                static_cast<float>(window_height) - static_cast<float>(m_current_mouse_y);
            ...
            GLfloat ctrlpoints[6][3] = {
                {static_cast<GLfloat>(from_x), static_cast<GLfloat>(from_y), 0.0},
                {static_cast<GLfloat>(from_x + 25), static_cast<GLfloat>(from_y), 0.0},
                {static_cast<GLfloat>(from_x + 100), static_cast<GLfloat>(from_y), 0.0},
                {static_cast<GLfloat>(to_x - 110), static_cast<GLfloat>(to_y), 0.0},
                {static_cast<GLfloat>(to_x - 35), static_cast<GLfloat>(to_y), 0.0},
                {static_cast<GLfloat>(to_x), static_cast<GLfloat>(to_y), 0.0}};
            glMap1f(GL_MAP1_VERTEX_3, 0.0, 1.0, 3, 6, &ctrlpoints[0][0]);
            glEnable(GL_MAP1_VERTEX_3);
            glBegin(GL_LINE_STRIP);
...
void opengl_view::draw_block_body(shader_block* Block, const double X, const double Y)
{
    ...
    const unsigned long max_properties = std::max(
        Block->input_count(), static_cast<unsigned long>(Block->m_outputs.size()));

    // set minimal block height
    const double height1 =
        m_services->is_rolled(Block)
            ? width
            : (width * (1.0 / 3.7) * static_cast<double>(max_properties));
    ...
    {
        points.push_back(radius * std::cos(angle) + xcoord);
        points.push_back(radius * std::sin(angle) + ycoord);
        points.push_back(0);
    }
...
void opengl_view::draw_rolled_block_body(
    shader_block* Block,
    const double X,
    const double Y)
{
...
void opengl_view::draw_block_name(
    ...
    const double Y)
{
    ...
    drawGlText(
        ...
        (float) (Y + .05),
        0,
        (m_font_size + 0.5) * m_size * 0.75,
        true);
}

void opengl_view::draw_block_properties(
    const shader_block* Block,
    const double X,
    const double Y,
    positions_t& PropertyPositions,
    const bool Selection)
{
    ...
    for (auto input = Block->m_inputs.begin(); input != Block->m_inputs.end(); ++input)
    {
        ...
        if (Selection)
        {
            ...
            m_property_indices.insert(
                std::make_pair(m_property_index, std::make_pair(Block, input->m_name)));
            ++m_property_index;
        }

        std::string type = Block->input_type(input->m_name);
        bool shader_param =
            (Block->is_shader_parameter(input->m_name) && !Block->m_root_block);

        // show property name
        glColor4f(0.65, 0.77, 0.97, 1.0); // #a5c5f7 light blue
        drawGlText(
            input->m_name,
            start_x + property_size * 1.25,
            start_y - 0.135,
            0,
            m_font_size * m_size * 0.75,
            false);
        ...
        if (input->m_multi_operator_parent_name.empty())
            draw_property(
                ...
                start_y,
                property_size,
                input->is_multi_operator());

        PropertyPositions.insert(std::make_pair(
            shrimp::io_t(Block->name(), input->m_name),
            position(start_x + property_size / 2, start_y - property_size / 2)));
         ...
         ++output)
    {
        if (Selection)
        {
            glLoadName(m_property_index);
            m_property_indices.insert(
                std::make_pair(m_property_index, std::make_pair(Block, output->m_name)));
            ++m_property_index;
        }
        ...
        draw_property(
            output->m_name, type, shader_param, start_x, start_y, property_size);
        PropertyPositions.insert(std::make_pair(
            shrimp::io_t(Block->name(), output->m_name),
            position(start_x + property_size / 2, start_y - property_size / 2)));
...
void opengl_view::draw_property(
    ...
    const bool Multi)
{
    ...
    if ("selected" == Type)
    {
        ...
        drawGlText(
            Name, X + Size * 1.25, Y - 0.135, 0, m_font_size * m_size * 0.75, false);
    }
    ...
    else if ("point" == Type)
    {
        ...
        {
            const double angle =
                static_cast<double>(i) * 2 * M_PI / static_cast<double>(sections);
            glVertex3d(
                (X + Size / 2.0) + radius * cos(angle),
                (Y - Size / 2.0) + radius * sin(angle),
                0);
        }
        glEnd();
    }
...
void opengl_view::draw_groups()
{
    ...
    for (auto& m_group_position : m_group_positions)
    {
        const int group = m_group_position.first;
        const double x  = m_group_position.second.position_x;
        const double y  = m_group_position.second.position_y;
        ...
        drawGlText(
            name, x - 0.3f, y + 0.4f, 0, (m_font_size + 0.5) * m_size * 0.75, false);
    }
}

void opengl_view::draw_group_body(int Group)
{
    auto g = m_group_positions.find(Group);
