...
double opengl_view::fit_scene(int window_width, int window_height)
{
    ...
    for (auto block_i = block_list.begin(); block_i != block_list.end(); ++block_i)
    {
    ...
    const double view_ratio =
        static_cast<double>(m_view_width) / static_cast<double>(m_view_height);
...
void opengl_view::move_active_group(const double XOffset, const double YOffset)
{
    ...
    for (int group : groups)
    {
        ...
        shrimp::shader_blocks_t group_blocks = m_services->get_group_blocks(group);
        for (auto group_block : group_blocks)
        {
            group_block->m_position_x += XOffset;
            group_block->m_position_y += YOffset;
        }
    }
}
...
void opengl_view::box_selection(int window_width, int window_height)
{
    ...
    double from_y =
        static_cast<float>(window_height) - static_cast<float>(m_start_drag_y);
    ...
    for (auto block : block_list)
    {
        ...
        if (!group)
        {
            ...
            const unsigned long max_properties = std::max(
                block->input_count(),
                (unsigned long) block->m_outputs
                    .size()); // cast required by some unusual compilers (e.g. gcc version 4.1.3 20070929 (prerelease))
            ...
            const double height =
                (height1 < m_min_block_height) ? m_min_block_height : height1;
            ...
            if (Fx < Tx && Ty < Fy)
            {
                ...
                if (point_inside(
                        block->m_position_x + width / 2,
                        block->m_position_y - height / 2,
                        Fx,
                        Ty,
                        Tx,
                        Fy))
        ...
        if (group)
        {
            auto p = m_group_positions.find(group);
...
shrimp::io_t opengl_view::get_under_mouse_property(const int mouse_x, const int mouse_y)
{
    ...
    if (selected_block)
        return std::make_pair(
            m_property_indices[closest].first->name(),
            m_property_indices[closest].second);
    ...
    const bool ctrl_key_down)
{
    ...
    if (ctrl_key_down)
    {
        if (m_under_mouse_block)
        {
            ...
            m_services->set_block_selection(
                m_under_mouse_block, !m_services->is_selected(m_under_mouse_block));
        }
        if (m_under_mouse_group)
        {
            if (m_services)
            {
                m_services->set_group_selection(
                    m_under_mouse_group,
                    !m_services->is_group_selected(m_under_mouse_group));
            }
        }
    }
    ...
    else if (shift_key_down)
    {
        ...
        {
            // toggle block selection
            m_services->set_block_selection(m_under_mouse_block, true);
        }
        if (m_under_mouse_group)
        {
            m_services->set_group_selection(m_under_mouse_group, true);
        }
    }
...
void opengl_view::mouse_left_button_drag(
    ...
    const bool alt_key_down)
{
    ...
    const auto mouse_move_x = static_cast<double>(m_current_mouse_x - m_last_mouse_x);
    const auto mouse_move_y = static_cast<double>(m_current_mouse_y - m_last_mouse_y);
    ...
    if (m_mouse_click <= 1)
    {
        if (m_active_property.first != "")
        {
            ...
            else
            {
                ...
                {
                    // connect properties
                    shader_block* input_block =
                        m_services->get_block(m_connection_start.first);
                    shader_block* output_block =
                        m_services->get_block(m_active_property.first);
        ...
        //select block
            ...
            (m_under_mouse_block || m_under_mouse_group) && !(shift_key_down || ctrl_key_down))
        {
            m_services->clear_selection();
            if (m_under_mouse_block)
            {
